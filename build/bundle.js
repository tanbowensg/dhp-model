...
exports.filterLabels = exports.nameFilter = exports.formatSize = exports.parseImageAddress = exports.post = exports.get = undefined;
...
var _http = __webpack_require__(370);
...
function cutUrl(url) {
  ...
  var path = '/' + nohttp.split('/').slice(1, 99999).join('/');
  return { host: host, path: path };
}

function get(url, configs) {
  var hostPath = cutUrl(url);
  var options = {
  ...
  return new Promise(function (resolve, reject) {
    var req = _http2.default.get(options, function (res) {
      var json = '';
...
function parseImageAddress(imgAddress) {
  ...
  var splitedAddress = imgAddress.split('/');
  ...
  var _name = splitedAddress.pop();
  // the first element in splitedName should be the image name with tag or digest
  if (_name.indexOf('@') > 0 && _name.indexOf(':') > 0) {
    // ubuntu:latest@xxxxx
    var _name$split$0$split = _name.split('@')[0].split(':'),
        _name$split$0$split2 = _slicedToArray(_name$split$0$split, 2),
        image = _name$split$0$split2[0],
        tag = _name$split$0$split2[1];

    parsed.imgName = image;
    parsed.tag = tag || '';
  } else if (_name.indexOf('@') > 0) {
    // digest string has ':', so we detect '@' character first
    var _name$split = _name.split('@'),
        _name$split2 = _slicedToArray(_name$split, 2),
        _image = _name$split2[0],
        digest = _name$split2[1];

    parsed.imgName = _image;
    parsed.digest = digest;
  } else {
    var _name$split3 = _name.split(':'),
        _name$split4 = _slicedToArray(_name$split3, 2),
        _image2 = _name$split4[0],
        _tag = _name$split4[1];

    parsed.imgName = _image2;
    parsed.tag = _tag || '';
  }

  var daocloudNamespace = parsed.nameSpace ? parsed.nameSpace + '/' : '';
  ...
  parsed.name = [parsed.registry, parsed.nameSpace, parsed.imgName].filter(function (i) {
    return i;
  }).join('/');
...
function formatSize(num, unit, hodor) {
  ...
  if (unit.toUpperCase() === 'B' && num < 1024 || unit.toUpperCase() === 'P' && num >= 1) {
    return '' + num + unit.toUpperCase();
  }

  var inputUnintIndex = units.indexOf(unit.toUpperCase());

  if (num >= 1) {
    var relIndex = Math.floor(Math.log(num) / Math.log(1024));
    var _number = (num / Math.pow(1024, Math.floor(relIndex))).toFixed(PRECISION);
    var _outputUnit = units[relIndex + inputUnintIndex];
    return '' + _number + _outputUnit;
  }
  var number = (num * 1024).toFixed(PRECISION);
  var outputUnit = units[inputUnintIndex - 1];
...
function filterLabels(originalLabels) {
  var filterdLabels = _lodash2.default.clone(originalLabels);
  _lodash2.default.forEach(_constant.PRIVATE_LABELS, function (privateLabel) {
...
// 自定义图标的 label
var CUSTOM_ICON_LABEL = 'io.daocloud.dce.icon';

// 这些 label 需要对用户隐藏掉
var PRIVATE_LABELS = ['io.daocloud.dce.authz.owner', 'io.daocloud.dce.authz.tenant', 'com.docker.stack.namespace', 'io.daocloud.dce.volume.opt.type', 'io.daocloud.dce.volume.opt.o', 'io.daocloud.dce.volume.opt.device', 'io.daocloud.dce.version', 'io.daocloud.dce.updated-at', 'io.daocloud.dce.system', 'io.daocloud.dce.project', 'io.daocloud.dce.job', 'io.daocloud.dce.template', 'com.docker.swarm.constraints', 'com.docker.swarm.id', 'com.docker.swarm.reschedule-policies', CUSTOM_ICON_LABEL];

var API_URL = 'http://192.168.100.205';
...
var _Rx = __webpack_require__(16);
...
var _infoStream = __webpack_require__(94);
...
var services$$ = hub$$.filter(function (jobs) {
  return jobs.includes('service');
});
var containers$$ = hub$$.filter(function (jobs) {
  return jobs.includes('container');
});
var tasks$$ = hub$$.filter(function (jobs) {
  return jobs.includes('task');
});
var networks$$ = hub$$.filter(function (jobs) {
  return jobs.includes('task');
});
var apps$$ = hub$$.filter(function (jobs) {
  return jobs.includes('app');
});
var registries$$ = hub$$.filter(function (jobs) {
  return jobs.includes('registry');
});
...
(function zero() {
  _Rx2.default.Observable.combineLatest((0, _infoStream.getApiInfo)())
...
// 启动 socket
α$$.subscribe(function (apiInfo) {
  // socket$
  _Rx2.default.Observable.create(function (observer) {
    // 由于一开始没有 job 推送，为了初始化整个数据池，先手动推送一个 init job
    _socket2.default.connect(apiInfo.StreamRoom);
    _socket2.default.bind('observable', function (job) {
      observer.next(job);
    });
    // 初始化一下
    observer.next('all');
...
exports.servicesVm$$ = exports.services$ = undefined;
...
var _service = __webpack_require__(85);

var _service2 = _interopRequireDefault(_service);

var _service3 = __webpack_require__(91);

var _hub = __webpack_require__(18);

var _hub2 = _interopRequireDefault(_hub);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var servicesVm$$ = new _Rx2.default.BehaviorSubject().filter(function (v) {
  return v;
});
...
// 输入应该是后端发回来的 raw 的数据
// 返回的是可以直接供前端渲染的数据
var services$ = _hub2.default.services$$.concatMap(function () {
  return _Rx2.default.Observable.fromPromise(_service2.default.list());
})
// 格式化
.map(function (services) {
  return _lodash2.default.map(services, function (service) {
    return new _service3.Service(service);
  });
});

services$.subscribe(servicesVm$$);

servicesVm$$.subscribe(function (services) {
...
exports.networksVm$$ = exports.networks$ = undefined;
...
var _network3 = __webpack_require__(90);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var networksVm$$ = new _Rx2.default.BehaviorSubject().filter(function (v) {
  return v;
});
...
function classifyNetworks(networks) {
  ...
  networks.forEach(function (net) {
    if (!(net.name === 'host' || net.name === 'none' || net.name === 'dce_default' || net.name === 'docker_gwbridge')) {
...
// 一收到 socket，就直接去拿列表，并且格式化
var networks$ = _hub2.default.networks$$.concatMap(function () {
  return _Rx2.default.Observable.fromPromise(_network2.default.list());
})
// 格式化
.map(function (networks) {
  return _lodash2.default.map(networks, function (network) {
    return new _network3.Network(network);
  });
}).map(classifyNetworks);

networks$.subscribe(networksVm$$);

networksVm$$.subscribe(function (networks) {
...
exports.tasksVm$$ = exports.tasks$ = undefined;
...
var _task3 = __webpack_require__(92);

var _serviceStream = __webpack_require__(35);
...
var tasksVm$$ = new _Rx2.default.BehaviorSubject().filter(function (v) {
  return v;
});

// 一收到 socket，就直接去拿列表，然后加以格式化的 Observable
var tasks$ = _hub2.default.tasks$$.concatMap(function () {
  return _Rx2.default.Observable.fromPromise(_task2.default.list());
})
...
.zip(_serviceStream.servicesVm$$, function (tasks, services) {
  // 由于 task 里面只有服务的 id，所以要从对应的服务列表中根据 id 查到名字，然后塞进去再格式化
  var serviceIdNameMap = _lodash2.default.fromPairs(_lodash2.default.map(services, function (s) {
    return [s.id, s.name];
  }));
  return _lodash2.default.map(tasks, function (task) {
    var serviceName = serviceIdNameMap[task.ServiceID];
    // 格式化 task
    return new _task3.Task(task, { serviceName: serviceName });
  });
});

tasks$.subscribe(tasksVm$$);

tasksVm$$.subscribe(function (tasks) {
...
var ArrayObservable_1 = __webpack_require__(13);
var ArrayLikeObservable_1 = __webpack_require__(225);
var iterator_1 = __webpack_require__(23);
var Observable_1 = __webpack_require__(0);
var observeOn_1 = __webpack_require__(41);
var observable_1 = __webpack_require__(26);
...
exports.getAppDetail = exports.appsVm$$ = exports.apps$ = undefined;
...
var _app3 = __webpack_require__(87);

var _taskStream = __webpack_require__(45);

var _serviceStream = __webpack_require__(35);

var _networkStream = __webpack_require__(44);
...
// 输入应该是后端发回来的 raw 的数据
// 返回的是可以直接供前端渲染的数据
var appsVm$$ = new _Rx2.default.BehaviorSubject().filter(function (v) {
  return v;
});
// 一收到 socket，就直接去拿列表
var apps$ = _hub2.default.apps$$.concatMap(function () {
  return _Rx2.default.Observable.fromPromise(_app2.default.list());
}).zip(_taskStream.tasksVm$$, _serviceStream.servicesVm$$, function (apps, tasks, services) {
  return _lodash2.default.map(apps, function (app) {
    return new _app3.App(app, tasks, services);
  });
});

apps$.subscribe(appsVm$$);

appsVm$$.subscribe(function (apps) {
...
function getAppDetail(appName) {
  return appsVm$$.map(function (apps) {
    return _lodash2.default.find(apps, function (app) {
      return app.name === appName;
    });
  }).combineLatest(_networkStream.networksVm$$, function (app, networks) {
    console.log(networks.all.length);
    return new _app3.AppDetail(app, networks.all);
  });
}
...
var _container3 = __webpack_require__(88);
...
var containersVm$$ = new _Rx2.default.BehaviorSubject().filter(function (v) {
  return v;
});
...
// 输入应该是后端发回来的 raw 的数据
// 返回的是可以直接供前端渲染的数据
var containers$ = _hub2.default.containers$$.concatMap(function () {
  return _Rx2.default.Observable.fromPromise(_container2.default.list());
})
// 格式化
.map(function (containers) {
  return _lodash2.default.map(containers, function (container) {
    return new _container3.Container(container);
  });
});

containers$.subscribe(containersVm$$);

containersVm$$.subscribe(function (containers) {
...
exports.registriesVm$$ = exports.registries$ = undefined;
...
var registriesVm$$ = new _Rx2.default.BehaviorSubject().filter(function (v) {
  return v;
});
...
var registries$ = _hub2.default.registries$$.concatMap(function () {
  return _Rx2.default.Observable.fromPromise(_registry2.default.list());
});
// 格式化
...
registriesVm$$.subscribe(function (registries) {
...
function list() {
  ...
  return (0, _util.get)(_constant.API_URL + '/api/apps', configs);
}

function detail(appName, configs) {
  return (0, _util.get)(_constant.API_URL + '/api/apps/' + appName, configs);
}

// 停止应用
function stop(appName) {
  return (0, _util.post)(_constant.API_URL + '/api/apps/' + appName + '/stop');
}

// 重启应用
function restart(appName) {
  return (0, _util.post)(_constant.API_URL + '/api/apps/' + appName + '/restart');
}
...
function list() {
  ...
  var nodePrefix = nodeId ? 'api/nodes/' + nodeId + '/docker/' : '';
  return (0, _util.get)(_constant.API_URL + '/' + nodePrefix + 'containers/json');
}
...
function apiInfo() {
  return (0, _util.get)(_constant.API_URL + '/api/info').then(function (res) {
    return res;
  });
}
...
function list() {
  ...
  var nodePrefix = nodeId ? 'api/nodes/' + nodeId + '/docker/' : '';
  return (0, _util.get)(_constant.API_URL + '/' + nodePrefix + 'networks');
}
...
function list() {
  ...
  return (0, _util.get)(_constant.API_URL + '/api/integrations?Catalog=registry');
}

// 获取内置的镜像工场
function getBuildinRegistry() {
  return (0, _util.get)(_constant.API_URL + '/api/settings/buildin-registry');
}

exports.default = {
  list: list,
  getBuildinRegistry: getBuildinRegistry
};
...
function list() {
  ...
  return (0, _util.get)(_constant.API_URL + '/services');
}

function detail(serviceId) {
  return (0, _util.get)(_constant.API_URL + '/services/' + serviceId);
}
...
function list() {
  ...
  return (0, _util.get)(_constant.API_URL + '/tasks');
}

function appTasks(appName) {
  return (0, _util.get)(this.API_URL + '/api/apps/' + appName + '/tasks');
}
...
exports.AppDetail = exports.App = undefined;
 ...
 */
var App = function () {
  function App(app, tasks, services) {
  ...
  _createClass(App, [{
    key: '_init',
    value: function _init(app, tasks, services) {
      // {
      //   name: String,
      //   tasks: Array,
      //   services: Array,
      ...
      this.servicesName = _lodash2.default.map(app.services, 'name').join('、');
      this.images = _lodash2.default.map(app.services, 'image.image').join('、');
    ...
    key: '_tasks',
    value: function _tasks(app, tasks) {
      return _lodash2.default.chain(app.Services).map('ID')
      // 这里就不对每个 task 重新发请求了，直接用 taskList 里的数据
      .map(function (serviceId) {
        return _lodash2.default.find(tasks, { serviceId: serviceId });
      }).value();
    }
    ...
    key: '_services',
    value: function _services(app, services) {
      return _lodash2.default.map(app.Services, function (service) {
        return _lodash2.default.find(services, { id: service.ID });
      });
    }
    ...
    key: '_servicesStateNum',
    value: function _servicesStateNum(app) {
      var result = {
        running: 0,
        stopped: 0,
        system: 0
      };
      _lodash2.default.forEach(app.services, function (serv) {
    ...
    key: '_hasAccessPoints',
    value: function _hasAccessPoints(app) {
      return _lodash2.default.chain(app.services).map(function (serv) {
        return serv.ports;
      }).flatten().some(function (v) {
        return v;
      }).value();
    ...
    key: '_updateAt',
    value: function _updateAt(app) {
      var time = 0;
      _lodash2.default.forEach(app.services, function (serv) {
        var value = serv.updatedAt.valueOf();
    ...
    key: '_globalServices',
    value: function _globalServices(app) {
      return _lodash2.default.chain(app.services).filter(function (serv) {
        return serv.mode.global;
      }).map('name').value();
    }
    ...
    key: '_allServicesAreGlobal',
    value: function _allServicesAreGlobal(app) {
    ...
    key: '_appTemplate',
    value: function _appTemplate(app) {
      var templateLabel = '';
      _lodash2.default.forEach(app.services, function (serv) {
        var _templateLabel = _lodash2.default.get(serv.spec, 'Labels[\'io.daocloud.dce.template\']', false);
    ...
    key: '_isRunning',
    value: function _isRunning(app) {
      var stateNum = this._servicesStateNum(app);
    ...
    value: function _cpu(app) {
      ...
      _lodash2.default.forEach(app.services, function (serv) {
        if (unlimited) {
          var _cpu = serv.resources.cpuLimit;
          var _replicas = serv.mode.replicas;
    ...
    value: function _memory(app) {
      ...
      _lodash2.default.forEach(app.services, function (serv) {
        if (unlimited) {
          var _memory = serv.resources.memLimitRaw;
          var _replicas = serv.mode.replicas;
      ...
      return (0, _util.formatSize)(memory, 'B');
    }
  }]);
...
var AppDetail = function () {
  function AppDetail(app, networks) {
  ...
  _createClass(AppDetail, [{
    key: '_init',
    value: function _init(app, networks) {
      var _this = this;

      // 把 app 里的属性复制出来
      _lodash2.default.forEach(app, function (val, key) {
        _this[key] = val;
      });
      // 把网络的名字塞到服务里
    ...
    key: '_networkName',
    value: function _networkName(app, networks) {
      return _lodash2.default.map(app.services, function (service) {
        _lodash2.default.forEach(service.networks, function (network) {
          network.name = _lodash2.default.find(networks, { id: network.id }).name;
        });
        return service;
      });
    }
...
var _i18n = __webpack_require__(46);
...
function splitStatus(state, str, require) {
  ...
  if (lang === 'cn' && require === 'status') {
    switch (state) {
      case 'created':
        result = '' + _i18n.CONTAINER.status.created;
        break;

      case 'restarting':
        result = '' + _i18n.CONTAINER.status.restarting + (words[1] || '');
        break;

      case 'running':
      case 'up':
        result = '' + _i18n.CONTAINER.status.running;
        break;

      case 'paused':
        result = '' + _i18n.CONTAINER.status.paused;
        break;

      case 'exited':
        result = '' + _i18n.CONTAINER.status.exited + (words[1] || '');
        break;

      case 'dead':
        result = '' + _i18n.CONTAINER.status.dead;
        break;
    ...
    if (state === 'restarting') {
      ...
      result = joined.replace('Less than a second', '1 秒').replace('hours', '小时').replace('minutes', '分钟').replace('seconds', '秒').replace('weeks', '周').replace('days', '天').replace('months', '月').replace('years', '年').replace('About', '大约').replace(' a ', '1').replace(' an ', '1').replace('hour', '小时').replace('minute', '分钟').replace(' ago', '以前');
    }
  } else {
    result = words.join(' ');
  }
...
var Container = function () {
  ...
  _createClass(Container, [{
    ...
    value: function _init(container) {
      ...
      this.imageParsed = (0, _util.parseImageAddress)(container.Image);
    ...
    key: '_name',
    value: function _name(container) {
    ...
    key: '_node',
    value: function _node(container) {
      // 主机信息原本也是在 Names 里的
      // 原本名字的格式： /主机名/容器名。
      var name = container.Names[0];
    ...
    key: '_image',
    value: function _image(container) {
      var words = container.Image.split('/');
...
var Job = function () {
  ...
  _createClass(Job, [{
    key: '_init',
    value: function _init(job) {
      _lodash2.default.merge(this, job);
      this.name = this._name(job);
      this.entity = this._entity(job);
      this.state = _i18n.JOB.state[job.State];
    ...
    key: '_name',
    value: function _name(job) {
      if (_i18n.JOB.name.hasOwnProperty(job.Name)) {
        return _i18n.JOB.name[job.Name];
      }
      var name = job.Name.split('.').reverse().join('');
      _lodash2.default.forEach(_i18n.JOB.nameFragment, function (zh, en) {
    ...
    key: '_entity',
    value: function _entity(job) {
      if (_i18n.JOB.entity.hasOwnProperty(job.Entity.ObjectType)) {
        return _i18n.JOB.entity[job.Entity.ObjectType];
      }
    ...
    key: '_time',
    value: function _time(job) {
      var start = job.CreatedAt * 1000;
      var end = job.FinishedAt * 1000;
      return {
        start: (0, _moment2.default)(start).fromNow(),
        finish: end ? (0, _moment2.default)(end).fromNow() : 'till now',
        cost: end ? Math.abs(_moment2.default.duration(end - start, 'ms').seconds()) + ' \u79D2' : Math.abs(_moment2.default.duration((0, _moment2.default)() * 1000 - start, 'ms').seconds()) + ' \u79D2'
      };
    }
  }]);
...
var Network = function () {
  ...
  _createClass(Network, [{
    ...
    value: function _init(network) {
      ...
      this.Created = (0, _moment2.default)(network.Created);
      this.labels = (0, _util.filterLabels)(network.Labels);
    }
    ...
    key: '_name',
    value: function _name(network) {
      if (network.Driver === 'overlay') {
        return network.Name;
      }
      return (0, _util.nameFilter)(network.Name, 'full');
    }
    ...
    key: '_node',
    value: function _node(network) {
      if (network.Driver === 'overlay') {
        return '全局';
      }
      return (0, _util.nameFilter)(network.Name, 'host');
    }
    ...
    key: '_app',
    value: function _app(network) {
      return (0, _util.nameFilter)(network.Name, 'app');
    }
  }]);
...
var _shellQuote = __webpack_require__(371);

var _shellQuote2 = _interopRequireDefault(_shellQuote);
...
var Service = function () {
  ...
  _createClass(Service, [{
    ...
    value: function _init(service) {
      ...
      //   inStoppedMenu: Bool,
      // }
      var constraints = this._constraints(service);
    ...
    key: '_image',
    value: function _image(service) {
      var imageAddr = _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.Image', '');
      var _image = (0, _util.parseImageAddress)(imageAddr);
      return _image;
    }
    ...
    key: '_originalImageName',
    value: function _originalImageName(service) {
      return _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.Image', '');
    }
    ...
    key: '_hostname',
    value: function _hostname(service) {
      return _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.Hostname', '');
    }
    ...
    key: '_createdAt',
    value: function _createdAt(service) {
      return (0, _moment2.default)(service.CreatedAt);
    }
    ...
    key: '_updatedAt',
    value: function _updatedAt(service) {
      return (0, _moment2.default)(service.UpdatedAt);
    }
    ...
    key: '_appName',
    value: function _appName(service) {
      var labels = _lodash2.default.get(service, 'Spec.Labels', {});
      var isSystem = !!labels['io.daocloud.dce.system'];
      if (isSystem) return '';
      var appName = labels['com.docker.stack.namespace'];
    ...
    key: '_labels',
    value: function _labels(service) {
      var originalLabels = _lodash2.default.get(service, 'Spec.Labels', {});
      return (0, _util.filterLabels)(originalLabels);
    }
    ...
    key: '_modeValue',
    value: function _modeValue(service) {
    ...
    key: '_resources',
    value: function _resources(service) {
      var result = {};
      // 处理之前的数据
      result.cpuLimitRaw = _lodash2.default.get(service, 'Spec.TaskTemplate.Resources.Limits.NanoCPUs', 0);
      result.cpuReservationRaw = _lodash2.default.get(service, 'Spec.TaskTemplate.Resources.Reservations.NanoCPUs', 0);
      result.memLimitRaw = _lodash2.default.get(service, 'Spec.TaskTemplate.Resources.Limits.MemoryBytes', 0);
      result.memReservationRaw = _lodash2.default.get(service, 'Spec.TaskTemplate.Resources.Reservations.MemoryBytes', 0);
    ...
    key: '_updateConfig',
    value: function _updateConfig(service) {
      var result = {};
      result.parallelism = _lodash2.default.get(service, 'Spec.UpdateConfig.Parallelism', 0);
      result.failureAction = _lodash2.default.get(service, 'Spec.UpdateConfig.FailureAction', 'pause');
      result.delay = _lodash2.default.get(service, 'Spec.UpdateConfig.Delay', 0) / 1e6;
      result.period = _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.StopGracePeriod', 0) / 1e6;
      return result;
    }
    ...
    key: '_containerSpec',
    value: function _containerSpec(service) {
      // 命令
      function cmd(serv) {
        var _cmd = _lodash2.default.get(serv, 'Spec.TaskTemplate.ContainerSpec.Command', []);
        return _shellQuote2.default.quote(_cmd);
      }

      // 命令参数
      function args(serv) {
        var _args = _lodash2.default.get(serv, 'Spec.TaskTemplate.ContainerSpec.Args', []);
        return _shellQuote2.default.quote(_args);
      }

      // 组
      function groups(serv) {
        var _groups = _lodash2.default.get(serv, 'Spec.TaskTemplate.ContainerSpec.Groups', []) || [];
      ...
      // return {Object} 键值对
      function envs(serv) {
        return _lodash2.default.chain(serv).get('Spec.TaskTemplate.ContainerSpec.Env', []).map(function (item) {
          return item.split('=');
        }).fromPairs().value();
      }
      ...
      return {
        ...
        envs: envs(service),
        user: _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.User', ''),
        dir: _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.Dir', ''),
        labels: (0, _util.filterLabels)(_lodash2.default.get(service, 'Spec.Labels', {})),
        containerLabels: (0, _util.filterLabels)(_lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.Labels', {})),
        tty: _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.TTY', false)
      };
    }
    ...
    key: '_hostsMapping',
    value: function _hostsMapping(service) {
      return _lodash2.default.chain(service).get('Spec.TaskTemplate.ContainerSpec.Hosts', []).map(function (h) {
        return h.split(' ');
      }).fromPairs().value();
    }
    ...
    key: '_endpoint',
    value: function _endpoint(service) {
      // TODO VirtualIPs 真的是从这个字段里获得的吗？————安然
      var _virtualIPs = _lodash2.default.get(service, 'Endpoint.VirtualIPs', []);
      var _ports = _lodash2.default.get(service, 'Endpoint.Spec.Ports', []);

      // 统一变量命名规则: camelCase
      var ports = _ports.map(function (port) {
        return {
      ...
      var virtualIPs = _virtualIPs.map(function (item) {
        return {
          addr: item.Addr,
      ...
      return {
        ...
        mode: _lodash2.default.get(service, 'Endpoint.Spec.Mode', 'dnsrr'),
        virtualIPs: virtualIPs,
        ports: ports,
        // 开放端口 - 非负载均衡端口
        openPorts: ports.filter(function (p) {
          return p.publishMode === 'host';
        }),
        // 负载均衡端口
        lbPorts: ports.filter(function (p) {
          return p.publishMode === 'ingress';
        })
      };
    }
    ...
    key: '_constraints',
    value: function _constraints(service) {
      var result = {
        nodeId: '',
        nodeLabel: ''
      };
      var constraints = _lodash2.default.get(service, 'Spec.TaskTemplate.Placement.Constraints', []);

      _lodash2.default.forEach(constraints, function (p) {
    ...
    key: '_networks',
    value: function _networks(service) {
      var oldNetworks = _lodash2.default.get(service, 'Spec.Networks', []);
      var newNetworks = _lodash2.default.get(service, 'Spec.TaskTemplate.Networks', []);

      var networksIndexed = {};
      _lodash2.default.forEach(oldNetworks, function (aliases, target) {
        if (!networksIndexed[target]) {
          networksIndexed[target] = aliases;
        }
      });

      _lodash2.default.forEach(newNetworks, function (_ref) {
        ...
        // 合并旧的配置
        networksIndexed[Target] = _lodash2.default.concat(networksIndexed[Target], Aliases);
      });

      var networks = [];
      _lodash2.default.forEach(networksIndexed, function (aliases, target) {
        networks.push({
          Target: target,
          Aliases: aliases
        });
      ...
      return networks.map(function (net) {
        return { id: net.Target, aliases: net.Aliases };
      });
    }
    ...
    key: '_storage',
    value: function _storage(service) {
      var _mountsList = _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.Mounts', []);
      return _lodash2.default.map(_mountsList, function (m) {
        var driver = _lodash2.default.get(m, 'VolumeOptions.DriverConfig.Name', '');
        var driverOpts = _lodash2.default.get(m, 'VolumeOptions.DriverConfig.Options', {});
        var labels = _lodash2.default.get(m, 'VolumeOptions.Labels', {});
        var noCopy = _lodash2.default.get(m, 'VolumeOptions.NoCopy', false);
        return {
          type: m.Type,
          source: m.Source,
          target: m.Target,
    ...
    key: '_dnsConfig',
    value: function _dnsConfig(service) {
      // array
      var result = {
        nameservers: _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.DNSConfig.Nameservers', []),
        options: _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.DNSConfig.Options', []),
        search: _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.DNSConfig.Search', [])
      };
    ...
    key: '_highAvail',
    value: function _highAvail(service) {
      var _restartPolicy = _lodash2.default.get(service, 'Spec.TaskTemplate.RestartPolicy', {});
      var mode = this._mode(service);
    ...
    key: '_healthCheck',
    value: function _healthCheck(service) {
      var _healthCheck = _lodash2.default.get(service, 'Spec.TaskTemplate.ContainerSpec.Healthcheck', {});
      var result = {
        cmd: '',
        activate: true,
        test: _lodash2.default.get(_healthCheck, 'Test', []),
        interval: (_lodash2.default.get(_healthCheck, 'Interval', 0) || 0) / 1e9,
        timeout: (_lodash2.default.get(_healthCheck, 'Timeout', 0) || 0) / 1e9,
        retries: _lodash2.default.get(_healthCheck, 'Retries', 0)
      };

      var prefix = result.test[0] || '';
    ...
    key: '_fsMounts',
    value: function _fsMounts(service) {
      var storages = this._storage(service);
      return _lodash2.default.filter(storages, function (item) {
    ...
    key: '_linkedVolumes',
    value: function _linkedVolumes(service) {
      var storages = this._storage(service);
      return _lodash2.default.filter(storages, function (item) {
    ...
    key: '_logDriver',
    value: function _logDriver(service) {
      // 默认的日志驱动为 json-file
      var name = _lodash2.default.get(service, 'Spec.TaskTemplate.LogDriver.Name', 'json-file');
      var options = _lodash2.default.get(service, 'Spec.TaskTemplate.LogDriver.Options', {});
    ...
    key: '_isSystem',
    value: function _isSystem(service) {
      return !!_lodash2.default.get(service.Spec.Labels, 'io.daocloud.dce.system');
    }
    ...
    key: '_isRunning',
    value: function _isRunning(service) {
      var mode = this._mode(service);
    ...
    key: '_isStopped',
    value: function _isStopped(service) {
      var mode = this._mode(service);
    ...
    key: '_inRunningMenu',
    value: function _inRunningMenu(service) {
      var mode = this._mode(service);
      return (mode.global || mode.replicas) && !_lodash2.default.get(service.Spec.Labels, 'io.daocloud.dce.system');
    }
    ...
    key: '_inStoppedMenu',
    value: function _inStoppedMenu(service) {
      var mode = this._mode(service);
      return !mode.global && mode.replicas === 0 && !_lodash2.default.get(service.Spec.Labels, 'io.daocloud.dce.system');
    }
  }]);
...
var Task = function () {
  ...
  _createClass(Task, [{
    key: '_init',
    value: function _init(task, serviceName) {
      this.slot = task.Slot;
      this.desiredState = task.DesiredState;
      this.createdAt = (0, _moment2.default)(task.CreatedAt);
      this.id = task.ID;
      ...
      // 注意：依赖 this._image
      this.imgName = (0, _util.parseImageAddress)(this._image(task)).imgName;
    }
  }, {
    key: '_status',
    value: function _status(task) {
      var result = {};
      var CATEGORIES = {
        new: 'updating',
        allocated: 'updating',
      ...
      result.timestamp = (0, _moment2.default)(task.Status.Timestamp);
      result.desiredState = task.DesiredState;
      return result;
    }
  }, {
    key: '_containerId',
    value: function _containerId(task) {
    ...
    key: '_image',
    value: function _image(task) {
      if (task.Spec && task.Spec.ContainerSpec && task.Spec.ContainerSpec.Image) {
...
var _restify = __webpack_require__(79);
...
var _appStream = __webpack_require__(76);
...
var _containerStream = __webpack_require__(77);

var _registryStream = __webpack_require__(78);
...
exports.apiInfo$ = exports.getApiInfo = undefined;
...
var apiInfo$ = new _Rx2.default.BehaviorSubject().filter(function (v) {
  return v;
});

function getApiInfo() {
  var observable = _Rx2.default.Observable.fromPromise(_info2.default.apiInfo());
  observable.subscribe(function (apiInfo) {
...
var _socket = __webpack_require__(372);
...
var socket = void 0;
var callbackCancellers = {};
// 连接到任务的 socket.io
function connect(streamRoom) {
  socket = (0, _socket2.default)(_constant.API_URL + '/', {
...
function bind(action, callback) {
  if (!callbackCancellers.hasOwnProperty(action)) {
    var socketFunc = function socketFunc(job) {
      job = new _job.Job(job);
      callback(job);
    };
...
var Observable_1 = __webpack_require__(0);
var from_1 = __webpack_require__(253);
...
var Observable_1 = __webpack_require__(0);
var concatMap_1 = __webpack_require__(278);
...
var Observable_1 = __webpack_require__(0);
var mergeScan_1 = __webpack_require__(306);
...
var Observable_1 = __webpack_require__(0);
var min_1 = __webpack_require__(307);
...
var Observable_1 = __webpack_require__(0);
var pairwise_1 = __webpack_require__(308);
...
var Observable_1 = __webpack_require__(0);
var partition_1 = __webpack_require__(309);
...
var Observable_1 = __webpack_require__(0);
var pluck_1 = __webpack_require__(310);
...
var Observable_1 = __webpack_require__(0);
var publish_1 = __webpack_require__(311);
...
var Observable_1 = __webpack_require__(0);
var publishBehavior_1 = __webpack_require__(312);
...
var Observable_1 = __webpack_require__(0);
var publishReplay_1 = __webpack_require__(314);
...
var Observable_1 = __webpack_require__(0);
var sample_1 = __webpack_require__(319);
...
var Observable_1 = __webpack_require__(0);
var sampleTime_1 = __webpack_require__(320);
...
var Observable_1 = __webpack_require__(0);
var scan_1 = __webpack_require__(321);
...
var Observable_1 = __webpack_require__(0);
var sequenceEqual_1 = __webpack_require__(322);
...
var Observable_1 = __webpack_require__(0);
var share_1 = __webpack_require__(323);
...
var Observable_1 = __webpack_require__(0);
var skipUntil_1 = __webpack_require__(326);
...
var Observable_1 = __webpack_require__(0);
var startWith_1 = __webpack_require__(328);
...
var Observable_1 = __webpack_require__(0);
var take_1 = __webpack_require__(333);
...
var Observable_1 = __webpack_require__(0);
var takeLast_1 = __webpack_require__(334);
...
var Observable_1 = __webpack_require__(0);
var throttle_1 = __webpack_require__(337);
